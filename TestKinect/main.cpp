...
Vector4 setToZero(Vector4 &vector1, char m);
Vector4 turnCoord(Vector4 &vector1, char m, double theta);
int getTime(){
				...
				for (int i = 0; i < NUI_SKELETON_COUNT; i++)
				{
						...
						skeletonFrame.SkeletonData[i].eSkeletonPositionTrackingState[NUI_SKELETON_POSITION_SHOULDER_CENTER] != NUI_SKELETON_POSITION_NOT_TRACKED)
					{
						...
						for (int j = 0; j < NUI_SKELETON_POSITION_COUNT; j++)
						{
							...
							if (j == 7)
							{
								thrdataKinect << "Kinect: i=" << i << ",j=" << j << ":" << skeletonFrame.SkeletonData[i].SkeletonPositions[j].w <<
							...
							if (j == 7)
							{
								thrdataKinect << "Nao: i= " << i << ",j=" << j << ":" << coordnateOfNao[j].x << "," << coordnateOfNao[j].y << "," << coordnateOfNao[j].z
						...
						angleTemp[1] = getAngle(setToZero(getDiff(coordnateOfNao[5], coordnateOfNao[4]), 'y'),getDiff(coordnateOfNao[5], coordnateOfNao[4]));
						if (getDiff(coordnateOfNao[5], coordnateOfNao[4]).y < 0.0)
							angles[1] = -angleTemp[1];
						else
							angles[1] = angleTemp[1];
						cout << "angleTemp[1] = " << angleTemp[1] << endl;
						thrdataNao << "angles[1] = " << angles[1] * 57.296 << endl;
						...
						if (getDiff(coordnateOfNao[6], coordnateOfNao[5]).z > 0.0)
